...
export async function GET(req: Request) {
  try {
    ...
    const clientProductsDir = path.join(publicDir, 'uploads', 'clients', 'thepartystore');
    const generalUploadsDir = path.join(publicDir, 'uploads');

    const allItems: MediaAssetItem[] = [];

    // Scan the client's party store directory and general uploads
    await scanDir(clientProductsDir, publicDir, 3, allItems);
    await scanDir(generalUploadsDir, publicDir, 2, allItems);
