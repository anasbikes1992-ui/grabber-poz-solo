...
export async function POST(req: Request) {
  try {
    ...
    if (!['dry-run', 'persist-dry-run'].includes(action)) {
      return NextResponse.json({ success: false, error: 'Unsupported WooCommerce staging action' }, { status: 400 });
    }

    const csv = String(body.csv || '');
    assertCsvSize(csv);

    const sourceNamespace = String(body.sourceNamespace || 'thepartystore-legacy-woocommerce');
    ...
    const persisted =
      action === 'persist-dry-run'
        ...
        : null;
    const staged = persisted?.staged || stageWooCommerceCatalog(csv);
