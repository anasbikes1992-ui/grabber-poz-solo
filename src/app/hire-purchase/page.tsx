    ...
    <div className="space-y-6">
      <div className="flex justify-between gap-4 items-start">
        <div>
          ...
          <p className="text-xs text-muted-foreground">EMI contracts via /api/hire-purchase</p>
        </div>
          ...
          <tbody className="divide-y divide-zinc-800">
            {contracts.map((c) => (
              <tr key={c.id}>
                ...
                <td className="py-2 text-muted-foreground">{c.itemName}</td>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title="New HP contract" as="form" onSubmit={create}>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label htmlFor="hp-price" className="text-xs font-semibold block mb-1">Cash price</label>
              <input id="hp-price" type="number" value={form.totalCashPrice} onChange={(e) => setForm((f) => ({ ...f, totalCashPrice: Number(e.target.value) }))} className="w-full px-2 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-sm font-mono" />
            </div>
            <div>
              <label htmlFor="hp-down" className="text-xs font-semibold block mb-1">Down</label>
              <input id="hp-down" type="number" value={form.downPayment} onChange={(e) => setForm((f) => ({ ...f, downPayment: Number(e.target.value) }))} className="w-full px-2 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-sm font-mono" />
            </div>
            <div>
              <label htmlFor="hp-months" className="text-xs font-semibold block mb-1">Months</label>
              <input id="hp-months" type="number" value={form.totalMonths} onChange={(e) => setForm((f) => ({ ...f, totalMonths: Number(e.target.value) }))} className="w-full px-2 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-sm font-mono" />
            </div>
          </div>
