...
-- ====================================================================
-- DEPRECATED AS SCHEMA SSOT (W1-10):
--   Canonical schema is src/db/schema.ts. Prefer:
--     npm run db:push
--   or drizzle-kit generate/migrate.
--   This SQL may drift; use only for greenfield Supabase bootstraps, then
--   reconcile against schema.ts. Do not edit this file as the source of truth.
-- ====================================================================
-- This script resets and provisions tables, custom enum types,
-- foreign keys, performance indexes, and initial seed configuration.
-- Run this directly in the Supabase SQL Editor only when intentional.
-- ====================================================================
...
-- SECURITY: never GRANT ALL on schema to anon. App server uses DATABASE_URL
-- (postgres/service_role). PostgREST anon must not have DML on commerce tables.
-- Schema SSOT is src/db/schema.ts — regenerate this SQL from Drizzle when columns drift
-- (see docs/correction.md W1-10).
